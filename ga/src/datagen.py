...
data = load_df_with_json(path=PATH_TRAIN, json_cols=json_cols, dtype_dict=dtype_dict)
test = load_df_with_json(path=PATH_TEST, json_cols=json_cols, dtype_dict=dtype_dict)
...
data.to_csv(DATA_FE, index=False)
test.to_csv(TEST_FE, index=False)
